...
const MostrarLista = ({ jsonData }) => {
  const [searchTerm, setSearchTerm] = useState('');

  // Verifica si jsonData existe antes de acceder a sus propiedades
  ...
  // Verifica si jsonData es null antes de aplicar el método filter
  ...
  return (
    <div className="container">
      <h2>Mostrar Datos</h2>
      <div className="mb-3">
        {/* Agrega un input para la búsqueda con clases de Bootstrap */}
        <input
          type="text"
              ...
              {filteredData.map((row, rowIndex) => (
                <tr key={rowIndex}>
                  {row.map((cell, cellIndex) => (
                    <td key={cellIndex} className={cellIndex === 0 ? 'primera-celda' : ''}>
                      {cellIndex === 2 && typeof cell === 'number' ? `$${cell.toFixed(2)}` : cell}
                    </td>
                  ))}
                </tr>
              ))}
